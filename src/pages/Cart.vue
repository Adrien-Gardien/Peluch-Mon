    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="!isEmpty" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-4">
          <div v-for="item in cartItems" :key="`${item.pokemon.id}-${item.selectedVariant}`" 
            ...
            <div class="flex items-center space-x-6">
              ...
              <div class="flex items-center space-x-4">
                <div class="flex items-center border border-gray-300 rounded-lg">
                  <button @click="updateQuantity(item.pokemon.id, item.quantity - 1, item.selectedVariant)" 
                          class="px-3 py-2 text-gray-600 hover:text-gray-800">-</button>
                  <span class="px-4 py-2 border-x border-gray-300">{{ item.quantity }}</span>
                  <button @click="updateQuantity(item.pokemon.id, item.quantity + 1, item.selectedVariant)" 
                          class="px-3 py-2 text-gray-600 hover:text-gray-800">+</button>
                </div>
                <button @click="removeFromCart(item.pokemon.id, item.selectedVariant)" 
...
import { useCart } from '../composables/useStore';

const cart = useCart();

const cartItems = computed(() => cart.items.value);
const totalItems = computed(() => cart.totalItems.value);
const formattedTotal = computed(() => cart.formattedTotal.value);
const isEmpty = computed(() => cart.isEmpty.value);

const updateQuantity = (pokemonId: number, quantity: number, variant: 'normal' | 'shiny' = 'normal') => {
  cart.updateQuantity(pokemonId, quantity, variant);
};

const removeFromCart = (pokemonId: number, variant: 'normal' | 'shiny' = 'normal') => {
  cart.remove(pokemonId, variant);
};
</script> 